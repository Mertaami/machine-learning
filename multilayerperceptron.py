...
class MultiLayerPerceptron():
    ...
    @staticmethod
    def load(filename):
        array = np.load(open(filename, 'rb'), allow_pickle=True)
    ...
    def error(self, output, expected) -> float:
    ...
    def learn(self, space, test_space, iterations=1000, test_iterations=100, epochs=100, save=None):
        mean_errors = []
        mean_accuracy = []
        begin = time.time()
...
if __name__ == "__main__":
    ...
    mean_errors, mean_accuracy = mlp.learn(space, test_space, iterations=1000, epochs=1000, save='mnist.npy')
